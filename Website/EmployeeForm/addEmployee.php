...
<body id="main_body" >
	...
	<div id="form_container">
		...
		<h1><a>Employee</a></h1>
		<form id="employee" class="appnitro"  method="post" action="addEmployee.php">
...
<?php
	...
	if (isset($_POST['submit'])){
		...
		//FOR FIELDS THAT NEEDED NOT TO BE NULL
		$result1 = mysqli_query($con,"INSERT INTO `ideal home`.`employee` (
		`Street` ,
		`Street Num` ,
		`Postal Code` ,
		`Name` ,
		...
		VALUES (
		'".$_POST["element_3"]."', '".$_POST["element_4"]."', '".$_POST["element_5"]."', ".$_POST["element_1_1"].", ".$_POST["element_1_2"].", ".$_POST["element_6"].", ".$_POST["element_8"].",".$_POST["element_9"].", ".$_POST["element_2"].", ".$_POST["element_10"]."
		);");
		...
		else if ( !empty($war) ) {echo "<h3> $war->message </h3><p></p>";}
		else { echo '<h3 class="correct"> Employee '.$_POST['element_1_1'].' ' .$_POST['element_1_2']. ' succesfully added. <p></p> </h3>'; }
		// close connection*/
 ...
 ?>
		...
		<span>
			<input id="element_8_1" name="element_8" class="element radio" type="radio" value="M" />
<label class="choice" for="element_8_1" >Male</label>
<input id="element_8_2" name="element_8" class="element radio" type="radio" value="F" />
<label class="choice" for="element_8_2" >Female</label>
		...
		<div>
			<input id="element_2" name="element_9" class="element text medium" type="text" maxlength="255" value=""/> 
		</li>		<li id="li_3" >
		...
		<?php
			...
			if (mysqli_num_rows($result) > 0) {
				...
				echo '<option name ="element_10" value="'.$row["Employee_ID"].'">'.$row['Name'].' '.$row['Surname'].'</option>
			';
			    }
			} else {
			    echo "0 results";
