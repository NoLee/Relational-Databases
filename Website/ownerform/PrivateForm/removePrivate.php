...
<body id="main_body" >
	...
	<div id="form_container">
		...
		<h1><a>Remove Private</a></h1>
		<form id="form_982670" class="appnitro"  method="post" action="removePrivate.php">
					<div class="form_description">
					<a href=../../template.html> <img src="../../home.png" alt="Home" id="home_img"> </a>  
			<h2>Remove Private</h2>
			<p>Please insert the name of the private, and then select the correct one.</p>
		</div>						
			<ul >
...
<?php
	...
	if (isset($_POST['submit'])){	
		...
		$select = "SELECT p.Owner_ID, p.Name, p.Surname, o.Street, o.`Street Num`, o.`Postal Code`, o.`E-Mail`, o.Telephone, o.`Tax Registration Number`
			FROM `private` as p
		...
		if (mysqli_num_rows($result) > 0) {
		    ...
		    echo " </tr> </table> </li>";
		    echo '	</br><li id="li_3" >
				<label class="description" for="element_2">Insert Owner ID to delete (from the above results)</label>
				<div>
	...
	if (isset($_POST['delete_button'])){	
		$result = mysqli_query($con, "DELETE FROM `ideal home`.`owner` WHERE `owner`.`Owner_ID` = ".$_POST["element_2"]);
		if (mysqli_affected_rows($con)>0){
			echo '<h3 class="correct"> Successfully deleted </h3>';
		}
		else {
			echo '<h3 class="wrong"> No private with such ID </h3>';
		}
	}
?>	
	</ul>
