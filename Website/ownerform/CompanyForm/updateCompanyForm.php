...
<title>Update Company Owner</title>
...
<body id="main_body" >
	...
	<div id="form_container">
		...
		<h1><a>Update Company Owner</a></h1>
		<form id="employee" class="appnitro"  method="post" action="updateCompanyForm.php">
					<div class="form_description">
					<a href=../../template.html> <img src="../../home.png" alt="Home" id="home_img"> </a>  
			<h2>Update Company Owner</h2>
			<p>Please insert the Company Owner info</p>
		</div>						
			<ul >
...
<?php
	...
	if (isset($_POST['submit'])){
		...
		if ( empty(mysqli_error($con)) && empty($war)  ){
			$result3 = mysqli_query($con,"UPDATE `ideal home`.`company` SET `Owner_ID`=".$_POST['ID_to_update']." ,
			`Company Name`=".$_POST["element_1"]." ,
			`Company Type`=".$_POST["element_2"]." WHERE `company`.`Owner_ID` =".$_POST['ID_to_update'].";");
		}
		$war =mysqli_get_Warnings($con) ;	
		...
		else { echo '<h3 class="correct"> Company Owner '.$_POST['element_1'].' ' .$_POST['element_2']. ' succesfully updated. <p></p> </h3>'; }
	...
	//Company's Data
	$result = mysqli_query($con,"SELECT * FROM `company` WHERE `company`.`Owner_ID` = ".$_POST["ID_to_update"].";"); // ID_to_update IS FROM THE PREVIOUS PAGE (updateEmployee)
	$rowC = mysqli_fetch_assoc($result); //We have one row from the SELECT, we get the data with $row["column_name"]
	mysqli_close($con);
 ?>
		...
		<div>
			<input id="element_1" name="element_1" class="element text medium" type="text" maxlength="255" value="<?php echo $rowC['Company Name']; ?>"/> 
		</div> 
		</li>		<li id="li_2" >
		<label class="description" for="element_2">Company Type </label>
		<div>
			<input id="element_2" name="element_2" class="element text medium" type="text" maxlength="255" value="<?php echo $rowC['Company Type']; ?>"/> 
		</div><p class="guidelines" id="guide_2"><small>Describe The Company In A Few Words</small></p> 
		</li>		<li id="li_3" >
