...
<body id="main_body" >
	...
	<div id="form_container">
		...
		<h1><a>Remove Contract</a></h1>
		<form id="form_982670" class="appnitro"  method="post" action="removeContract.php">
					<div class="form_description">
					<a href=../template.html> <img src="../home.png" alt="Home" id="home_img"> </a>  
			<h2>Remove Contract</h2>
			<p>Please insert the name of the customer involded in the contract, and then select the correct one.</p>
		</div>						
			<ul >
...
<?php
	...
	if (isset($_POST['submit'])){	
		...
		$select = "SELECT c.Contract_ID, w.Name as CustomerName, w.Surname as CustomerSurname, e.Street as EstateStreet, e.`Street Num` as EstateStreetNum, c.Starts, c.Duration, c.Rent, c.`Payment Method` 
			FROM `contract` as c
			INNER JOIN `estate` as e
			ON c.Estate_ID=e.Estate_ID
			INNER JOIN `customer` as w
		...
		if (mysqli_num_rows($result) > 0) {
		    ...
		    echo " </tr> </table> </li>";
		    echo '	</br><li id="li_3" >
				<label class="description" for="element_2">Insert Contract ID to delete (from the above results)</label>
				<div>
	...
	if (isset($_POST['delete_button'])){	
		$result = mysqli_query($con, "DELETE FROM `ideal home`.`contract` WHERE `contract`.`Contract_ID` = ".$_POST["element_2"]);
		if (mysqli_affected_rows($con)>0){
			echo '<h3 class="correct"> Successfully deleted </h3>';
		}
		else {
			echo '<h3 class="wrong"> No contract with such ID </h3>';
		}
	}
?>	
	</ul>
