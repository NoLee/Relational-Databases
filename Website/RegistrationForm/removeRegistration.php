...
<title>Cancel A Registration</title>
...
<body id="main_body" >
	...
	<div id="form_container">
		...
		<h1><a>Cancel A Registrationt</a></h1>
		<form id="form_982670" class="appnitro"  method="post" action="removeRegistration.php">
					<div class="form_description">
					<a href=../template.html> <img src="../home.png" alt="Home" id="home_img"> </a>  
			<h2>Cancel A Registration</h2>
			<p>Please insert the name of the customer that his/her registration is about to be canceled, and then select the correct one.</p>
		</div>						
			<ul >
...
<?php
	...
	if (isset($_POST['submit'])){	
		...
		$select = "SELECT r.Customer_ID, c.Name as CustomerName, c.Surname as CustomerSurname, r.`Registration Date`
		...
		if (mysqli_num_rows($result) > 0) {
		    ...
		    echo " </tr> </table> </li>";
		    echo '	</br><li id="li_3" >
				<<label class="description" for="element_2">Insert Customer ID to delete (from the above results)</label>
				<div>
	...
	if (isset($_POST['delete_button'])){	
		$result = mysqli_query($con, "DELETE FROM `ideal home`.`registration` WHERE `registration`.`Customer_ID` = ".$_POST["element_2"]);
		if (mysqli_affected_rows($con)>0){
			echo '<h3 class="correct"> Successfully deleted </h3>';
		}
		else {
			echo '<h3 class="wrong"> No customer with such ID </h3>';
		}
	}
?>	
	</ul>
