...
<body id="main_body" >
	...
	<div id="form_container">
		...
		<h1><a>Cancel A visit</a></h1>
		<form id="form_982670" class="appnitro"  method="post" action="removeVisit.php">
					<div class="form_description">
					<a href=../template.html> <img src="../home.png" alt="Home" id="home_img"> </a>  
			<h2>Cancel A visit</h2>
			<p>Please insert the name of the customer that his/her visit is about to be canceled, and then select the correct one.</p>
		</div>						
			<ul >
...
<?php
	...
	if (isset($_POST['submit'])){	
		...
		$select = "SELECT v.Customer_ID, v.Estate_ID, c.Name as CustomerName, c.Surname as CustomerSurname, e.Street as EstateStreet, e.`Street Num` as EstateStreetNum, v.`Visit Date`
			FROM `visit` as v
			INNER JOIN `customer` as c
			ON v.Customer_ID=c.Customer_ID
			INNER JOIN `estate` as e
			ON v.Estate_ID=e.Estate_ID
			WHERE `Name` LIKE '".$_POST["element_1_1"]."' AND `Surname` LIKE '".$_POST["element_1_2"]."' ";
		...
		if (mysqli_num_rows($result) > 0) {
		    ...
		    echo " </tr> </table> </li>";
		    echo '	</br><li id="li_3" >
				<label class="description" for="element_2">Insert Customer ID & Estate ID to delete (from the above results)</label>	
				<span>
	...
	if (isset($_POST['delete_button'])){	
		$result = mysqli_query($con, "DELETE FROM `ideal home`.`visit` WHERE `visit`.`Customer_ID` = ".$_POST["element_2_1"]." AND `visit`.`Estate_ID` = ".$_POST["element_2_2"]."");
		if (mysqli_affected_rows($con)>0){
			echo '<h3 class="correct"> Successfully deleted </h3>';
		}
		else {
			echo '<h3 class="wrong"> No customer or estate with such ID </h3>';
		}
	}
?>	
	</ul>
