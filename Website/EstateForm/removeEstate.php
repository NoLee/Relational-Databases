...
<body id="main_body" >
	...
	<div id="form_container">
		...
		<h1><a>Remove Estate</a></h1>
		<form id="form_982670" class="appnitro"  method="post" action="removeEstate.php">
					<div class="form_description">
					<a href=../template.html> <img src="../home.png" alt="Home" id="home_img"> </a>  
			<h2>Remove Estate</h2>
			<p>Please insert the address of the Estate, and then select the correct one.</p>
		</div>						
			<ul >
...
<?php
	...
	if (isset($_POST['submit'])){	
		...
		$select = "SELECT e.Estate_ID, e.Street, e.`Street Num`, e.`Postal Code`, e.`Monthly Rent`, e.`Estate Type`, e.`Room Num`, e.`Square Meters`
			FROM `estate` as e
			 WHERE `Street` LIKE '".$_POST["element_1_1"]."' AND `Street Num` LIKE '".$_POST["element_1_2"]."' ";
		...
		if (mysqli_num_rows($result) > 0) {
		    ...
		    echo " </tr> </table> </li>";
		    echo '	</br><li id="li_3" >
				<label class="description" for="element_2">Insert Estate ID to delete (from the above results)</label>
				<div>
	...
	if (isset($_POST['delete_button'])){	
		$result = mysqli_query($con, "DELETE FROM `ideal home`.`estate` WHERE `estate`.`Estate_ID` = ".$_POST["element_2"]);
		if (mysqli_affected_rows($con)>0){
			echo '<h3 class="correct"> Successfully deleted </h3>';
		}
		else {
			echo '<h3 class="wrong"> No estate with such ID </h3>';
		}
	}
?>	
	</ul>
