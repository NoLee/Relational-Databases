...
<body id="main_body" >
	...
	<div id="form_container">
		...
		<form id="form_981137" class="appnitro"  method="post" action="updateEstateForm.php">
...
<?php
	...
	if (isset($_POST['submit'])){	
		...
		//FOR FIELDS THAT NEEDED NOT TO BE NULL
		$result1 = mysqli_query($con,"UPDATE `ideal home`.`estate` SET `Estate_ID`=".$_POST['ID_to_update'].",
		...
		`Employee_ID`=".$_POST["element_8"].", 
		`Owner_ID`=".$_POST["element_9"]." WHERE `Estate_ID`=".$_POST['ID_to_update'].";");
		...
		else { echo '<h3 class="correct"> Estate successfully updated. <p></p> </h3>'; }
		
	}
	$result = mysqli_query($con,"SELECT * FROM `estate` WHERE `estate`.`Estate_ID` = ".$_POST["ID_to_update"].";"); // ID_to_update IS FROM THE PREVIOUS PAGE (updateEmployee)
 ...
 ?>
		...
		<label class="description" for="element_8">Employee ID </label>
		<div>
		...
		<?php
			...
			if (mysqli_num_rows($result) > 0) {
			    ...
			    while($row = mysqli_fetch_assoc($result)) {
			        if ($emp_id==$row['Employee_ID']) { echo '<option name ="element_8" value="'.$row["Employee_ID"].'" selected="selected">'.$row['Name'].' '.$row['Surname'].'</option>';}
				 else{
					echo '<option name ="element_8" value="'.$row["Employee_ID"].'">'.$row['Name'].' '.$row['Surname'].'</option>';
				}
			    }
			} else {
			    echo "0 results";
		...
		<label class="description" for="element_9">Owner ID </label>
		<div>
		<select class="element select medium" id="element_9" name="element_9"> 
			<option value="" selected="selected"></option>
		<?php
			//Select data from a SELECT statement to show at the dropdown list
			$select = "SELECT `Owner_ID`,`Company Name`  FROM `company`";
			$result = mysqli_query($con, $select);

			if (mysqli_num_rows($result) > 0) {
			    // output data of each row
			    while($row = mysqli_fetch_assoc($result)) {
				    if ($own_id==$row['Owner_ID']) { echo '<option name ="element_9" value="'.$row["Owner_ID"].'" selected="selected">'.$row['Company Name'].' </option>';} 
				    else {
					echo '<option name ="element_9" value="'.$row["Owner_ID"].'">'.$row['Company Name'].' </option>';
				    }
			    }
			} else {
			    echo "0 results";
			}
			$select = "SELECT `Owner_ID`,`Name` ,`Surname` FROM `private`";
			$result = mysqli_query($con, $select);

			if (mysqli_num_rows($result) > 0) {
			    // output data of each row
			    while($row = mysqli_fetch_assoc($result)) {
				     if ($own_id==$row['Owner_ID']) { echo '<option name ="element_9" value="'.$row["Owner_ID"].'" selected="selected">'.$row['Name'].' '.$row['Surname'].' </option>';} 
				    else {
					echo '<option name ="element_9" value="'.$row["Owner_ID"].'">'.$row['Name'].' '.$row['Surname'].'</option>';
				    }
			    } 
			}
		       else {
